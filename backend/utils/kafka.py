...
class KafkaProducer:
    ...
    def stop(self):
        if self.producer:
            # flush() returns the number of messages still in queue.
            left = self.producer.flush(timeout=5.0)
            logger.info(f"Kafka producer stopped. {left} messages left in queue.")
            self.producer = None
    ...
    def send(self, topic: str, message: Dict[str, Any]):
        """
        Asynchronous-like send. confluent_kafka.produce is non-blocking.
